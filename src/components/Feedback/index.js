  ...
  const getAllData = async () => {
    try {
      ...
      const response = await getData(
        limit,
        skip,
        "lab-test/lab-test-rating?$populate[0][path]=user&$populate[0][select][0]=name&$populate[0][select][1]=avatar&$sort[createdAt]=-1"
      );
      const response2 = await getData(
        limit,
        skip,
        "lab-test-management/get-lab-rating-statistics"
      );
              ...
              <div className="w-max">
                <div className="mt-3 flex flex-col justify-center items-center">
                  <div className="text-3xl my-1 text-gray-800 font-bold">
                    {stats?.averageRating ? stats?.averageRating : 0}/5
                  </div>
                  <div>
