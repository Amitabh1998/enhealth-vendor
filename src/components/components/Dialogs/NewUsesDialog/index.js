...
export default function NewUsesDialog({
  isOpen,
  setIsOpen,
  setTableData,
  tableData,
  ...
  const saveHandler = async (e) => {
    ...
    try {
      if (names.length > 0) {
        ...
        const response = await addNewMultipleData("uses", payload);
  ...
  const addNameHandler = () => {
    if (name.trim() !== "") {
      ...
      if (_names.filter((item) => item.name === name).length > 0) {
        toast.error(`Use with name ${name} already exists.`);
      } else {
  ...
  //   e.preventDefault();
  //   if (name.trim() === "") {
  //     toast.error("Plaese enter a use", "bottom-right");
  //   } else {
  //     const _tableData = [...tableData];
  //     try {
  //       const response = await addNewUses(name);
