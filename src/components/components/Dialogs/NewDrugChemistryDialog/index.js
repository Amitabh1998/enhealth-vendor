...
import { addNewData, addNewMultipleData } from "@/api/common";
import { toast } from "react-toastify";

export default function NewDrugChemistryDialog({
  isOpen,
  setIsOpen,
  setTableData,
  tableData,
  ...
  const saveHandler = async (e) => {
    ...
    try {
      if (names.length > 0) {
        ...
        const response = await addNewMultipleData("drug-chemistry", payload);
